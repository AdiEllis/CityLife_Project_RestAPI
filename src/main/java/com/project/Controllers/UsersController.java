...
public class UsersController extends BaseController implements Validator {
    ...
    public BasicResponseModel addUser(@ModelAttribute("User") User user,
        ...
        if (!authUser.getAuthUserIsAdmin()) {
            System.out.println("user admin?" + user.getIsAdmin() + " " + "authUser admin?" + authUser.getAuthUserIsAdmin());
    ...
    public BasicResponseModel updateUserPassword(@RequestParam int oid, @RequestParam String password,
                                                 AuthUser authUser) {
        BasicResponseModel responseModel;
        if (!authUser.getAuthUserIsAdmin()) {
            responseModel = new BasicResponseModel(Definitions.NO_PERMISSIONS, Definitions.NO_PERMISSIONS_MSG);
            ...
            if (authUser.getAuthUserError() == null) {
                if (oid < 0 || password.length() == 0) {
                    responseModel = new BasicResponseModel(Definitions.MISSING_FIELDS, Definitions.MISSING_FIELDS_MSG);
                } else {

                    List<User> userRow = persist.getQuerySession().createQuery("FROM User WHERE oid = :oid")
                    ...
                    if (userRow.isEmpty()) {
                        ...
                        User oldUser = persist.loadObject(User.class, oid);
    ...
    public BasicResponseModel getUser(@RequestParam int oid, AuthUser authUser) {
        BasicResponseModel responseModel;
        if (!authUser.getAuthUserIsAdmin()) {
            responseModel = new BasicResponseModel(Definitions.NO_PERMISSIONS, Definitions.NO_PERMISSIONS_MSG);
        } else {
            if (authUser.getAuthUserError() == null) {
                if (oid < 0) {
                    ...
                    if (userRow.isEmpty()) {
                        ...
                        responseModel = new BasicResponseModel(persist.loadObject(User.class, oid));
                    }
                }
            } else if (authUser.getAuthUserError() == Definitions.INVALID_TOKEN) {
                responseModel = new BasicResponseModel(Definitions.INVALID_TOKEN, Definitions.INVALID_TOKEN_MSG);
    ...
    public BasicResponseModel deleteUser(@RequestParam int oid, @RequestParam boolean deleted,
                                         AuthUser authUser) {
        BasicResponseModel responseModel;
        if (!authUser.getAuthUserIsAdmin()) {
            responseModel = new BasicResponseModel(Definitions.NO_PERMISSIONS, Definitions.NO_PERMISSIONS_MSG);
            ...
            if (authUser.getAuthUserError() == null) {
                if (oid < 0) {
                    ...
                    if (userRow.isEmpty()) {
                        ...
                        User user = persist.loadObject(User.class, oid);
